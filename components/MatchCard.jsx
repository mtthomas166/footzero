...
export default function MatchCard({ match, tab }) {
  ...
  const timeStr = status === 'live'? (currentMinute? `${currentMinute}` : 'LIVE') : new Date(timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
  const timeLeft = status === 'upcoming'? getTimeLeft(timestamp) : '';
  const isMatchFavorited = homeFav || awayFav;

  return (
    <Link
      href={`/watch/${createMatchSlug(match)}?tab=${tab || 'live'}`}
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          {status === 'live'? (
            <span className="inline-flex items-center gap-1.5 rounded-full bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 px-2.5 py-0.5 text- font-extrabold text-red-600 dark:text-red-400">
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full bg-zinc-200/50 border border-zinc-300/40 dark:bg-zinc-900 dark:border-zinc-800 px-2.5 py-0.5 text- font-bold text-zinc-700 dark:text-zinc-300 shadow-sm">
              <Clock className="h-3 w-3" />
              {timeStr}
            </span>
          )}
          {isMatchFavorited && (
            <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 dark:bg-amber-400/10 border border-amber-200 dark:border-amber-400/20 px-2 py-0.5 text- font-extrabold text-amber-600 dark:text-amber-400 tracking-wider uppercase animate-pulse">
        ...
        {tournament && (
          <span className="text- font-bold text-zinc-600 dark:text-zinc-400 truncate max-w- flex items-center gap-1.5 bg-zinc-200/50 border border-zinc-300/40 dark:bg-zinc-950/60 dark:border-zinc-900 px-2.5 py-0.5 rounded-full shadow-sm" title={tournament}>
            {leagueLogo? (
      ...
      <div className="space-y-3.5 py-1">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            ...
            <span className="text- font-bold truncate text-zinc-800 dark:text-zinc-100 group-hover:text-violet-600 dark:group-hover:text-white transition-colors">{homeTeam.name}</span>
            <button
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
          ...
          {status === 'live' && displayHomeScore!== undefined && (
            <span className="text- font-black text-zinc-800 dark:text-white tabular-nums shrink-0 bg-zinc-100/85 dark:bg-zinc-950 px-2.5 py-0.5 rounded-md border border-zinc-200 dark:border-zinc-800/80 shadow-sm">{displayHomeScore}</span>
          )}
        </div>
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            <TeamBadge team={awayTeam} />
            <span className="text- font-bold truncate text-zinc-800 dark:text-zinc-100 group-hover:text-violet-600 dark:group-hover:text-white transition-colors">{awayTeam.name}</span>
            <button
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
          ...
          {status === 'live' && displayAwayScore!== undefined && (
            <span className="text- font-black text-zinc-800 dark:text-white tabular-nums shrink-0 bg-zinc-100/85 dark:bg-zinc-950 px-2.5 py-0.5 rounded-md border border-zinc-200 dark:border-zinc-800/80 shadow-sm">{displayAwayScore}</span>
          )}
        </div>
      </div>
      <div className="h-px bg-gradient-to-r from-transparent via-zinc-300 dark:via-zinc-800/80 to-transparent my-3.5" />
      <div className="flex items-center justify-between text-xs">
        <span className="text- text-zinc-500 dark:text-zinc-500 font-bold tracking-wide">
        ...
        {sources.length > 0? (
          ...
          <span className="inline-flex items-center gap-1 text- font-semibold text-zinc-500">
